  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out">
      <div className="mb-8 text-center sm:text-left">
        <h2 className="text-3xl font-bold bg-gradient-to-br from-slate-900 to-slate-700 bg-clip-text text-transparent">Create an account</h2>
        <p className="mt-2 text-sm text-slate-500 font-medium">Join us today and explore thousands of books.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          <div className="relative group">
            ...
            <input
              {...register('full_name')}
              type="text"
              className={`block w-full pl-11 pr-3 py-2.5 border ${errors.full_name ? 'border-red-300 focus:ring-red-400' : 'border-slate-200 focus:ring-slate-900/10'} rounded-xl focus:outline-none focus:ring-4 transition-all bg-white font-medium text-slate-900 placeholder:text-slate-400 placeholder:font-normal`}
              placeholder="John Doe"
            />
          </div>
          {errors.full_name && <p className="mt-1.5 text-sm text-red-500 font-medium animate-in fade-in">{errors.full_name.message}</p>}
        </div>
        ...
        <button
          ...
          className="w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-sm text-sm font-semibold text-white bg-slate-900 hover:bg-slate-800 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-900 disabled:opacity-70 disabled:cursor-not-allowed transition-all active:scale-[0.98] mt-2"
        >
          {mutation.isPending ? <Loader2 className="animate-spin h-5 w-5" /> : 'Create Account'}
        </button>
